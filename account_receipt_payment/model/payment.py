...
class AccountPayment(models.Model):
    ...
    def _compute_available_account_ids(self):
        ...
        for pay in self:
            if pay.not_contact:
                domain = [('user_type_id.type', 'not in', ('receivable', 'payable')), ('company_id', '=', pay.company_id.id),
                          ('id','not in',(pay.journal_id.default_account_id | pay.outstanding_account_id).ids)]
            else:
                domain = [('user_type_id.type', 'in', ('receivable', 'payable')), ('company_id', '=', pay.company_id.id)]
